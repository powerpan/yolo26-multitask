...
class _MultiTaskHeadProxy:
    """Makes ``model.model[-1]`` resolve to a single-task head for Ultralytics loss classes."""

    def __init__(self, seq: torch.nn.Sequential, head: torch.nn.Module) -> None:
        self._seq = seq
        self._head = head
...
class TaskViewModel(torch.nn.Module):
    """Thin wrapper so ``v8*Loss`` sees ``model.model[-1]`` as ``det`` / ``pose`` / ``seg``."""

    def __init__(self, base: torch.nn.Module, head_attr: str) -> None:
        super().__init__()
        self.add_module("base", base)
        self.head_attr = head_attr

    @property
    def model(self) -> _MultiTaskHeadProxy:
        m = self.base.model
        return _MultiTaskHeadProxy(m, getattr(m[-1], self.head_attr))

    def __getattr__(self, name: str):
        if name in {"base", "head_attr"}:
...
class E2EMultiTaskLoss:
    ...
    def __init__(self, model, w_det: float = 1.0, w_pose: float = 1.0, w_seg: float = 1.0) -> None:
        ...
        self.w_pose = w_pose
        self.w_seg = w_seg
        self.det_o2m = v8DetectionLoss(TaskViewModel(model, "det"), tal_topk=10)
        self.det_o2o = v8DetectionLoss(TaskViewModel(model, "det"), tal_topk=7, tal_topk2=1)
        self.pose_o2m = PoseLoss26(TaskViewModel(model, "pose"), tal_topk=10)
        self.pose_o2o = PoseLoss26(TaskViewModel(model, "pose"), tal_topk=7, tal_topk2=1)
        self.seg_o2m = v8SegmentationLoss(TaskViewModel(model, "seg"), tal_topk=10)
        self.seg_o2o = v8SegmentationLoss(TaskViewModel(model, "seg"), tal_topk=7, tal_topk2=1)
        self.updates = 0
        self.total = 1.0
        self.o2m = 0.8
    ...
    def _split_e2e(branch_out: Any) -> tuple[Any, Any | None]:
        ...
        return branch_out, None

    def parse_output(self, preds: Any) -> dict[str, Any]:
        preds = preds[1] if isinstance(preds, tuple) else preds
        det_m, det_o = self._split_e2e(preds["det"])
        pose_m, pose_o = self._split_e2e(preds["pose"])
        seg_m, seg_o = self._split_e2e(preds["seg"])
        return {"det_m": det_m, "det_o": det_o, "pose_m": pose_m, "pose_o": pose_o, "seg_m": seg_m, "seg_o": seg_o}
    ...
    def update(self) -> None:
        """Match ``E2ELoss`` o2m/o2o schedule (uses pose branch ``hyp.epochs``)."""
        self.updates += 1
        self.o2m = self.decay(self.updates)
        self.o2o = max(self.total - self.o2m, 0)

    def decay(self, x) -> float:
        epochs = max(self.pose_o2o.hyp.epochs - 1, 1)
