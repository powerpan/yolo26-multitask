...
from ultralytics.utils import DEFAULT_CFG, DEFAULT_CFG_DICT, LOGGER, WINDOWS, YAML, colorstr, emojis
...
class BaseModel(torch.nn.Module):
    ...
    def _apply(self, fn):
        ...
        if isinstance(m, MultiTask26):
            ...
            for sub in (m.det, m.pose, m.seg):
                sub.stride = fn(sub.stride)
                sub.anchors = fn(sub.anchors)
                sub.strides = fn(sub.strides)
        elif isinstance(
            m, Detect
...
class DetectionModel(BaseModel):
    ...
    def __init__(self, cfg="yolo26n.yaml", ch=3, nc=None, verbose=True):
        ...
        if isinstance(m, MultiTask26):
            ...
            for sub in (m.det, m.pose, m.seg):
                sub.training = True
            stride = torch.tensor([s / x.shape[-2] for x in _forward_mt(torch.zeros(1, ch, s, s))])
            m.stride = stride
            for sub in (m.det, m.pose, m.seg):
                sub.stride = stride.clone()
            self.stride = stride
            self.model.train()
            m.bias_init()
        elif isinstance(m, Detect):  # includes all Detect subclasses like Segment, Pose, OBB, YOLOEDetect, YOLOESegment
            s = 256  # 2x min stride
            m.inplace = self.inplace
...
class MultiTaskModel(DetectionModel):
    """YOLO26 multitask stack: shared trunk + :class:`MultiTask26` (det / pose / seg) with ``E2EMultiTaskLoss``."""

    def __init__(self, cfg="yolo26-multitask.yaml", ch=3, nc=None, verbose=True):
        super().__init__(cfg=cfg, ch=ch, nc=nc, verbose=verbose)
...
def parse_model(d, ch, verbose=True):
    ...
    for _k in ("nc_det", "nc_pose", "nc_seg"):
        if _k in d and isinstance(d[_k], str):
            with contextlib.suppress(ValueError, SyntaxError):
                d[_k] = ast.literal_eval(d[_k])
    nc_det = d.get("nc_det", nc)
    nc_pose = d.get("nc_pose", nc)
    nc_seg = d.get("nc_seg", nc)
    scale = d.get("scale")
    if scales:
        if not scale:
