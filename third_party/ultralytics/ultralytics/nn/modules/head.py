...
__all__ = "OBB", "Classify", "Detect", "MultiTask26", "Pose", "RTDETRDecoder", "Segment", "YOLOEDetect", "YOLOESegment", "v10Detect"
...
class MultiTask26(nn.Module):
    """YOLO26 multi-head: shared FPN features, separate Detect / Pose26 / Segment26 branches.

    Use this when **detection**, **pose**, and **segmentation** describe different semantic components of the same
    physical object: each branch has its **own class space** (``nc_det``, ``nc_pose``, ``nc_seg``).

    Training returns ``{"det": ..., "pose": ..., "seg": ...}`` where each value matches the corresponding single-task
    head output (including end-to-end nested ``one2many`` / ``one2one`` dicts when enabled).
    """

    legacy = False  # set by parse_model for YOLO26-style heads
    ...
    def __init__(
        ...
        self.nl = len(ch)
        self.det = Detect(nc_det, reg_max, end2end, ch)
        self.pose = Pose26(nc_pose, kpt_shape, reg_max, end2end, ch)
        self.seg = Segment26(nc_seg, nm, npr, reg_max, end2end, ch)
        self.inplace = True
        self.stride = torch.zeros(self.nl)

    @property
    def end2end(self) -> bool:
        return getattr(self.det, "end2end", False)

    @end2end.setter
    def end2end(self, value: bool) -> None:
        self.det.end2end = value
        self.pose.end2end = value
        self.seg.end2end = value

    def bias_init(self) -> None:
        """Initialize biases for all three heads (requires ``stride`` to be set)."""
        self.det.bias_init()
        self.pose.bias_init()
        self.seg.bias_init()
    ...
    def forward(self, x: list[torch.Tensor]) -> dict:
        det_out = self.det(x)
        pose_out = self.pose(x)
        seg_out = self.seg(x)
        if self.training:
            return {"det": det_out, "pose": pose_out, "seg": seg_out}
        return {"det": det_out, "pose": pose_out, "seg": seg_out}

    def fuse(self) -> None:
        """Fuse each branch for inference (drops one-to-many branches where applicable)."""
        self.det.fuse()
        self.pose.fuse()
        self.seg.fuse()
